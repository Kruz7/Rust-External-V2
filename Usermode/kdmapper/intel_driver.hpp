...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			Log(_xor_(L"[-] Failed to load ntdll.dll").c_str() << std::endl); //never should happens
			return false;
		}

		const auto NtAddAtom = reinterpret_cast<void*>(GetProcAddress(ntdll, _xor_("NtAddAtom").c_str()));
		if (!NtAddAtom)
		{
			Log(_xor_(L"[-] Failed to get export ntdll.NtAddAtom").c_str() << std::endl);
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		static uint64_t kernel_NtAddAtom = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, _xor_("NtAddAtom").c_str());
		if (!kernel_NtAddAtom) {
			Log(_xor_(L"[-] Failed to get export ntoskrnl.NtAddAtom").c_str() << std::endl);
			return false;
		}
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			Log(_xor_(L"[-] FAILED!: The code was already hooked!! another instance of kdmapper running?!").c_str() << std::endl);
			return false;
		}
