...
#define BaseNetworkable_c 53726152
#define OclusionCulling 54252080
#define ConvarCullingC 53830728
#define ConsoleSystem_Index_c 53823840
#define ConVar_Admin_c 53683056
...
bool SetupKernelThread(PVOID KBase, PVOID ThreadStartAddr)
{
    ...
    if (Thread)
    {
        ...
        *(PVOID*)((ULONG64)EThread + StartAddrOff) = FakeStartAddr;
...
#define oItemDefinition 0x18
#define oItemid 0x18
#define oHeld 0xB0
#define oActiveUID 0x7D0
#define oPlayerInventory 0x8C0
#define oContainerBelt 0x28
#define oBeltContentList 0x40
#define oRecoilProperties 0x2E0
#define oRecoilYawMin 0x18
#define oRecoilYawMax 0x1C
#define oRecoilPitchMin 0x20
#define oRecoilPitchMax 0x24
...
public:
    WeaponData* GetWeaponInfo(int Id)
    {
        ...
        const uintptr_t ItemList = safe_read(Belt + 0x40, uintptr_t);
    ...
    WeaponData* GetActiveWeapon()
    {
        ...
        for (int i = 0; i < 6; i++) {
            ActiveWeapon = GetWeaponInfo(i);
            if (ActiveWeapon && ActUID == ActiveWeapon->GetUID()) return ActiveWeapon;
        }
        return nullptr;
    }
};
...
r5:
    ...
    while (true) {
        ...
        if (!LocalPlayer.BasePlayer) continue;
        ...
        if (NORECOIL && LocalPlayer.BasePlayer && LocalPlayer.BasePlayer->GetActiveWeapon() && is_weapon()) {
            ...
            if (base_projectile && recoil_prop)
            {
        ...
        if (SPIDERMAN && LocalPlayer.BasePlayer)
        {
            uintptr_t base_movement = Read<uintptr_t>((uintptr_t)LocalPlayer.BasePlayer + 0x6A0);
            if (base_movement) safe_write(base_movement + 0xC8, 0.f, float);
        }
        if (ADMINFLAG && LocalPlayer.BasePlayer)
        {
            ...
            if (ConsoleSystemCommands)
            {
                ...
                for (int i = 0; i < 300; i++)
                {
                    ...
                    {
                        const uintptr_t Name = safe_read(ConsoleSystemCommand + 0x10, uintptr_t);
                        ...
                        if (Name)
                        {
                            ...
                            {
                                safe_write(ConsoleSystemCommand + 0x58, false, bool);
                            }
                        }
                    }
                }
            }
            auto player_flags = Read<uintptr_t>((uintptr_t)LocalPlayer.BasePlayer + 0x8B0);
            player_flags |= 4;
        ...
        Write<float>(static_fields2 + 0x0, ALWAYSDAY ? 12 : -1);
    }
